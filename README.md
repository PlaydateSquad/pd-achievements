...
2. Edit `_data/games.yml`, copying the commented template and supplying the requisite information for your game. You’ll need to supply the following (**fields are required unless otherwise indicated**):
   ...
   | `releaseDate`      | `date`      | The date your game released, in `YYYY-MM-DD` format. If your game hasn’t released yet, you may provide a release date up to 30 days in the future. <br><br> A "Soon" badge will appear on games that have a future release date. Unless you also provide a `firstAddedDate`, this is the date your achievements became available, so a "New" badge will appear for 30 days following it.                                                                                                                                                                                                    |
   | `firstAddedDate`   | `date?`     | (_optional_) The date achievements were _first_ added to your game, in `YYYY-MM-DD` format. Only needed if your game released before it supported achievements—leave it out if your achievements were there from day one. If your game has already released but doesn’t yet support achievements, provide a date up to 30 days in the future on which you expect an update with achievements to release. Defaults to the release date if none is provided. <br><br> A "Soon\*" badge will appear on released games whose achievements haven’t arrived yet, followed by a "New" badge for the 30 days after they do. |
   | `lastAddedDate`    | `date?`     | (_optional_) The date new achievements were _most recently_ added to your game, in `YYYY-MM-DD` format. Only needed if you’ve added achievements since they first appeared. Defaults to the first-added date (or the release date) if none is provided. <br><br> A "More" badge will appear on games that have added achievements within the past 30 days. A future date is not badged until it arrives—if your game has released but has no achievements yet, use `firstAddedDate` instead.                                                                             |
