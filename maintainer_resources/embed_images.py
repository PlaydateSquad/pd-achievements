...
def _bytesToB64(bytelist: list[int]) -> str:
	...
	leftover = 0
	for byt in bytelist:
		# Chip off each current byte into 6-bit values, put the leftover n bits at the starts of the next.
		# Since each byte is 8 bits, each time around the leftover is a little larger.
		bitcount = bitcount + 8
		...
		# After each 3 bytes, the leftovers have become large enough that it becomes a 6-bit value on its own.
		leftover = (byt & bitmask_a)
		if bitcount % 24 == 0:
			result.append(_BASE64[leftover])
			leftover = 0
	
	# Check to see if there's any unhandled leftovers, and return the result as a string.
	if bitcount % 24 != 0:
		result.append(_BASE64[leftover])
	return "".join(result)
...
def _getPattern(img: Image, sx: int, sy: int, wx: int, wy: int) -> list[int]:
	# Returns a rectangular chunck of the (greyscale, 1 byte-per-pixel) image.
	bytelist = []
	bit=0
	nextbyte=0
...
def _convertToBase64(variable_to_image_table: dict[str, str]) -> (dict[str, str], dict[str, str]):
	# Given an image-filename to variable-name mapping, returns a mapping of variable-name to b64-string per image.
	# The first part of the result-tuple is variable-name to base-64 string repressenting 1-bit (b/w) images.
	# The second part is the same, but for the alpha-channel of that image. (May not be present for each variable.)
	# NOTE: This bunches the image into 8x8 chunks, so it can efficiently be parsed via 'setPattern' on the Playdate.
	#       Each pixel w.r.t. a chunk is arranged left to right, top to bottom, same for the chunks w.r.t. the image.
	b64_per_variable = {}
...
def _embedBase64StringsToFile(b64_per_var: dict[str, str], opacity_b64_per_var: dict[str, str], lua_file: str) -> None:
	# Embeds base-64 strings into a lua-file that has 'autogenerated' markers to place such things between.
	# It is assumed the variable-assgignments are done within a table, so a comma is necesary in between.
	# Each table assigned to a named variable contains:
	# - '.pattern' to which the base-64 strings in `b64_per_var` are assigned, and
	# - (optionally) '.alpha', same but for the values of `opacity_b64_per_var` instead.
	writes=[]
	try:
...
if __name__ == "__main__":
	# For (some) technical details, see the comments in the functions above.
