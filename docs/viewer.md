...
The viewer module presents all the achievements in your game to the player. When launched, the viewer appears as a modal overlay and temporarily blocks updates and input to your game. Players can scroll through a list of all available achievements, see which they’ve earned or made progress towards, and exit when finished to return to your game.
...
> NOTE:
> To use the viewer, ensure all of the [required assets](../achievements/assets) are included in the "achievements/assets" directory of your game. Many of these assets are also used by `toasts.lua`. You can avoid a delay when launching the viewer by calling `achievements.viewer.initialize()` to preload the assets.

## Configuration

You can override the default configuration to adjust the appearance and behavior of the in-game viewer. See the schema below for a full list of properties you can change.
...
-- in response to some action, such as selecting a menu option
achievements.viewer.launch()
```
...
| ---------------------- | ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `assetPath`            | `string`                       | The path to the image/sound/font [assets](../achievements/assets) required by the module. Defaults to `"achievements/assets"`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `fadeColor`            | `playdate.graphics.color`      | The color to fade the background when the viewer launches. You can set this to `kColorWhite` if your game is mostly dark, or `kColorClear` to prevent a background fade. Defaults to `kColorBlack`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `enableAboutScreen`    | `bool`                         | When `true` the player will have the option to display an about screen with a QR code to learn more about Playdate Achievements. Defaults to `true`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `invertCards`          | `bool`                         | When `true` the viewer appearance is inverted to display white headers and black cards. (Achievement icons will display normally and won’t be inverted.) Defaults to `false`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `summaryMode`          | `count/percent/score/none`     | Determines how achievements are summarized in the header. Must be one of:<br><br>**`count`**: Display the number of unlocked achievements out of the total number (including optional ones), e.g. "3/12".<br><br>**`percent`**: Display a percentage completion, weighted by the `scoreValue` for each achievement, e.g. "25%".<br><br>**`score`**: Display the raw `scoreValue` earned out of the possible total, e.g. 60/100.<br><br>**`none`**: Omit the summary<br><br>Defaults to `count`.                                                                                                                                                                        |
| `disableBackground`    | `bool`                         | Set this to `true` to prevent a static captured background image of your game from appearing behind the viewer. You are responsible for setting an `updateFunction` and drawing something behind the viewer yourself.                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `showSlot`             | `number/string`                | Which save slot to show the current data from. Normally you will set this to `nil` to have it show achievements granted in the current save slot, but if you want to display a different slot's data you can do so here. Must be either a number between 1 and `achievements.saveSlots`, or the string `'combined'` to show the total data of all slots, or the boolean `false` to override a previously-selected showSlot value when calling launch() again. This setting supercedes `gameData`. Defaults to `nil`                                                                                                                                                    |
...
> NOTE: **Device State:** The viewer will backup and restore as much device state as possible to avoid affecting your game, including refresh rate, display scale, and input handlers. If you find that launching the viewer has a negative effect on your game, consider restoring any essential state by providing a `returnToGameFunction` in your config. For example, you may need to explicitly pause any active timers and then unpause them again when control returns to your game.
