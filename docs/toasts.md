...
> NOTE:
> To use toasts, ensure all of the [required assets](../achievements/assets) are included in the "achievements/assets" directory of your game. Many of these assets are also used by `viewer.lua`. You can avoid a delay when displaying your first toast by calling `achievements.toasts.initialize()` to preload the assets.

## Configuration

You can override the default configuration to adjust the appearance and behavior of toast notifications in your game. See the schema below for a full list of properties you can change.
...
| `miniMode`            | `bool`                    | When `true`, toasts are rendered in a miniature format with a smaller font and no description. Mini toasts take up substantially less space on screen, keeping more of your game visible. Note that toasts are always show mini if the display scale is greater than 1. Defaults to `false`.                                                                                                                                                                                                                                                                    |
...
| `shadowColor`         | `playdate.graphics color` | Shadows are rendered in `kColorBlack` by default. Set this to `kColorWhite` for a glow effect, or `kColorClear` to omit the shadow. Note that the shadow is never shown if the display scale is greater than 1.                                                                                                                                                                                                                                                                                                                                                 |
